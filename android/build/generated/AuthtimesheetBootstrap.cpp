...
#define TAG "ti.authtimesheet"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Authtimesheet_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Authtimesheet.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::AuthtimesheetBindings::lookupGeneratedInit(
...
static void Authtimesheet_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Authtimesheet_getBinding)->GetFunction());
}

static void Authtimesheet_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::AuthtimesheetBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry AuthtimesheetBinding = {
	"ti.authtimesheet",
	Authtimesheet_init,
	Authtimesheet_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_authtimesheet_AuthtimesheetBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.authtimesheet", &AuthtimesheetBinding);
	titanium::KrollBindings::addExternalLookup(&(::AuthtimesheetBindings::lookupGeneratedInit));
}
